...
import (
	...
	"github.com/longhorn/node-disk-manager/pkg/blkid"
...
func GetNewBlockDevices(disk *block.Disk, nodeName, namespace string) []*longhornv1.BlockDevice {
	...
	if fileSystemInfo.Type == "" {
		fileSystemInfo.Type = blkid.GetFileSystemType(disk.Name)
	}
	logrus.Debugf("debug: %v", fileSystemInfo)
	logrus.Debugf("debug: %v", disk.FileSystemInfo.MountPoint)
	logrus.Debugf("debug: %v", disk.FileSystemInfo.FsType)
	logrus.Debugf("debug: %v", disk.FileSystemInfo.IsReadOnly)
